...
usage! {
	...
	}
	{
		...
		["Sealing/Mining Options"]
			...
			ARG arg_gas_floor_target: (String) = "2000000", or |c: &Config| c.mining.as_ref()?.gas_floor_target.clone(),
			"--gas-floor-target=[GAS]",
			"Amount of gas per block to target when sealing a new block.",

			ARG arg_gas_cap: (String) = "0", or |c: &Config| c.mining.as_ref()?.gas_cap.clone(),
			"--gas-cap=[GAS]",
