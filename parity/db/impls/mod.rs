...
use ethcore::client::{ClientConfig, DatabaseBackend};
use kvdb::KeyValueDB;
...
use self::rocksdb_migration::{migrate as migrate_rocksdb, Error};

/// Migrates the database.
pub fn migrate(path: &Path, backend: &DatabaseBackend) -> Result<(), Error> {
	match *backend {
		DatabaseBackend::Lmdb => {
			debug!(target: "lmdb", "migration isn't supported");
			Ok(())
		},
		DatabaseBackend::RocksDB { ref db_compaction, .. } => {
			migrate_rocksdb(path, db_compaction)
		}
	}
}
...
pub fn restoration_db_handler(client_path: &Path, client_config: &ClientConfig) -> Box<BlockChainDBHandler> {
	let db_backend = helpers::client_db_config(client_path, &client_config.db_backend);
...
/// Open a new main DB.
pub fn open_db(client_path: &str, db_backend: &DatabaseBackend) -> io::Result<Arc<BlockChainDB>> {
	let backend = helpers::client_db_config(&Path::new(client_path), db_backend);
	open_database(client_path, backend)
}
