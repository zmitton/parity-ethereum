...
impl Provider for TestProvider {
	...
	{
		Some(request::transaction_index::Response {
			num: 100,
			hash: req.hash,
			index: 55,
		})
	...
	fn epoch_signal(&self, _req: request::epoch_signal::CompleteRequest) -> Option<request::epoch_signal::Response> {
		Some(request::epoch_signal::Response {
			signal: vec![1, 2, 3, 4],
		})
	}
...
fn credit_overflow() {
	...
	let requests = encode_single(Request::Headers(header::IncompleteRequest {
...
fn get_block_headers() {
	...
	let request = Request::Headers(header::IncompleteRequest {
		start: HashOrNumber::Number(1).into(),
		max: 10,
		skip: 0,
		reverse: false,
	...
	let response = {
		...
		let response = vec![Response::Headers(header::Response { headers })];
...
fn get_block_bodies() {
	...
	for i in 0..10 {
		...
		builder.push(Request::Body(block_body::IncompleteRequest {
			hash: hash.into(),
		})).unwrap();
		bodies.push(Response::Body(provider.client.block_body(block_body::CompleteRequest {
			hash: hash,
		}).unwrap()));
	}
	let req_id = 111;
...
fn get_block_receipts() {
	...
	for hash in block_hashes.iter().cloned() {
		builder.push(Request::Receipts(block_receipts::IncompleteRequest { hash: hash.into() })).unwrap();
		receipts.push(Response::Receipts(provider.client.block_receipts(block_receipts::CompleteRequest {
			hash: hash
		}).unwrap()));
	}
...
fn get_state_proofs() {
	...
	let mut builder = Builder::default();
	builder.push(Request::Account(account::IncompleteRequest {
		block_hash: H256::default().into(),
		address_hash: key1.into(),
	})).unwrap();
	builder.push(Request::Storage(storage::IncompleteRequest {
		block_hash: H256::default().into(),
		address_hash: key1.into(),
		key_hash: key2.into(),
	})).unwrap();
	...
	let response = {
		let responses = vec![
			Response::Account(provider.account_proof(account::CompleteRequest {
				block_hash: H256::default(),
				address_hash: key1,
			}).unwrap()),
			Response::Storage(provider.storage_proof(storage::CompleteRequest {
				block_hash: H256::default(),
				address_hash: key1,
				key_hash: key2,
			}).unwrap()),
...
fn get_contract_code() {
	...
	let request = Request::Code(contract_code::IncompleteRequest {
	...
	let request_body = make_packet(req_id, &requests);
	let response = {
		let response = vec![Response::Code(contract_code::Response {
...
fn epoch_signal() {
	...
	let req_id = 112;
	let request = Request::Signal(request::epoch_signal::IncompleteRequest {
	...
	let response = {
		let response = vec![Response::Signal(epoch_signal::Response {
			signal: vec![1, 2, 3, 4],
		})];
...
fn proof_of_execution() {
	...
	let req_id = 112;
	let mut request = Request::Execution(request::execution::IncompleteRequest {
		block_hash: H256::default().into(),
...
fn id_guard() {
	...
	let req = encode_single(Request::Headers(header::IncompleteRequest {
...
fn get_transaction_index() {
	...
	let request = Request::TransactionIndex(transaction_index::IncompleteRequest {
		hash: key1.into(),
	});
	...
	let request_body = make_packet(req_id, &requests);
	let response = {
		let response = vec![Response::TransactionIndex(transaction_index::Response {
			num: 100,
			hash: key1,
			index: 55,
		})];
